...
const playerList = [];
const answerList = [];
let ranNum = Math.floor(Math.random() * stuffApp.thingsArray.length);
...
stuffApp.events = function() {
	...
	$('form.thingButton').on('submit', function(e) {
			e.preventDefault();	
			$('.category').html(`<p class="animated fadeIn">${stuffApp.thingsArray[Math.floor(Math.random() * stuffApp.thingsArray.length)]}</p>`);
	});
